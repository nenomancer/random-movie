...
import { currentHistory, allCountries, currentMovie, activeTab, currentFilters, useFilters } from "../lib/stores";
import { API, DEFAULT_MOVIE, DOCUMENT_TITLE, LOCAL_SESSION_HISTORY_KEY, TAB_NAME } from "./constants";
...
export const filterQueries = derived(
    ...
    ([$useFilters, $currentFilters]) => {
        ...
        return buildFilterQuery({
            ...
            ratingFrom: $currentFilters.ratingFrom! > 0 ? $currentFilters.ratingFrom : undefined,
            ratingTo: $currentFilters.ratingTo! < 10 ? $currentFilters.ratingTo : undefined,
        });
    }
);
...
async function getMovie(movieId: number) {
    ...
    await fetch(`${API.MOVIE}/${movieId}`, API.OPTIONS)
        ...
        .then((data) => {
            ...
            document.title = `${kurac} | ${DOCUMENT_TITLE}`;
...
export function normalizeMovie(data: any) {
    ...
    const countries = get(allCountries);
    return {
        backdrop: data.backdrop_path ? `${API.POSTER}/${data.backdrop_path}` : undefined,
