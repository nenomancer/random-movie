...
class Randomizator {
  constructor() {
    ...
    this.firstTab = document.querySelector('.info[data-tab="first"]');
    this.secondTab = document.querySelector('.filters[data-tab="second"]');
    this.firstTabButton = document.querySelector(".tabs .first");
    this.secondTabButton = document.querySelector(".tabs .second");
    this.allGenres = [];
    this.allCountries = [];
  ...
  init() {
    this.mainScreen.classList.add("loading");
    this.generateCountries()
      .then(this.getGenres.bind(this))
      .then(this.fetchMovies.bind(this));

    console.log("here?");
  }

  fetchMovies(queries = "", totalPages = 500) {
    this.mainScreen.classList.add("loading");
    ...
    this.firstTab.classList.add("active");
    this.firstTabButton.classList.add("active");
    this.secondTab.classList.remove("active");
    this.secondTabButton.classList.remove("active");
    //tuka?
    let earliest = 1878;
    ...
    fetch(
      ...
      .then((response) => {
        ...
        this.displayResult(response.results);
      })
      .catch((err) => {
        console.error(err);
  ...
  getDirectorMovies(id) {
    fetch(
      ...
      .then((response) => {
        ...
        return this.displayResult(temp, temp.length);
      })
      .catch((err) => console.error(err));
  }
  ...
  getGenreMovies(id) {
    this.fetchMovies("&with_genres=" + id);
  }

  displayResult(data, maxIndex = 20) {
    ...
    data.forEach((movie, index) => {
      if (index === randomIndex) {
        ...
        //       temp.innerText = genre.name;
        //       temp.addEventListener("click", (e) =>
        //         getGenreMovies(genre.id).bind(this)
        //       );
        ...
        fetch("https://api.themoviedb.org/3/movie/" + movie.id, options)
          ...
          .then((data) => {
            ...
            this.mainScreen.classList.remove("loading");
          })
  ...
  bindTabToggles() {
    this.firstTabButton.addEventListener("click", (e) => {
      this.firstTabButton.classList.add("active");
      this.secondTabButton.classList.remove("active");
      this.firstTab.classList.add("active");
      this.secondTab.classList.remove("active");
    });
    this.secondTabButton.addEventListener("click", (e) => {
      this.secondTabButton.classList.add("active");
      this.firstTabButton.classList.remove("active");
      this.secondTab.classList.add("active");
      this.firstTab.classList.remove("active");
    });
  }
}

const rand = new Randomizator();
